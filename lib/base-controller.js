...
BaseController.prototype.getNextStep = function getNextStep(req, res) {
  var siteroot = (res && res.locals) ? res.locals.siteroot || '' : '';
  var next = siteroot + Controller.prototype.getNextStep.apply(this, arguments);
  if (req.params.action === 'edit' && !this.options.continueOnEdit) {
    next = req.baseUrl === '/' ? this.confirmStep : req.baseUrl + siteroot + this.confirmStep;
...
BaseController.prototype.getErrorStep = function getErrorStep(err, req) {
  /* ugh
   * res argument missing from call in Form.prototype.errorHandler
   */
  var siteroot = req.res && req.res.locals ? req.res.locals.siteroot || '' : '';
  var redirect = siteroot + Controller.prototype.getErrorStep.call(this, err, req);
